package com.harshini.hibernate.hibernateExcersise.filter;
...
public class JwtAuthFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        System.out.println("uri ====>  "+uri.equalsIgnoreCase("/user/authenticate"));
        if(!(uri.equalsIgnoreCase("/user/authenticate"))) {
            System.out.println(" The API request is not "+uri);
            if (!authHeader.isBlank() && authHeader.startsWith("Bearer ")) {
                token = authHeader.substring(7);
                uName = jwtService.extractUsername(token);
            }
            if (!uName.isBlank() && SecurityContextHolder.getContext().getAuthentication() == null) {
